...
.
├── config
│   └── config.yaml             # Workflow configuration (paths, parameters)
├── data
│   └── raw_data.csv            # Input data for the workflow
├── logs                          
├── results                     # Output directory for analysis results
├── smk-slurm.sh                # SLURM batch submission script
└── workflow
  ├── Snakefile                 # Main workflow orchestration file
  ├── envs
  │   └── smk-ex.yaml           # Conda environment specification
  ├── profiles
  │   └── slurm
  │       └── config.yaml       # SLURM executor configuration (account, partition, resources)
  ├── rules
  │   ├── all.smk               # Final output targets
  │   ├── analysis.smk          # Analysis workflow rules
  │   └── preprocess.smk        # Data preprocessing rules
  └── scripts                   # Scripts used in the workflow
    └── helpers.py              # Example Python analysis functions
```
...
- **workflow/scripts/**: Contains the scripts used in the workflow.
  - `helpers.py`: Example Python analysis functions called by workflow rules.
...
- A "base" snakemake environment, see the [snakemake instructions](https://snakemake.readthedocs.io/en/stable/getting_started/installation.html).
  - This is the environment you will use anytime you want to run snakemake commands.

```bash
# Create a base snakemake conda environment
